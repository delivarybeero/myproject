...
<script>
// دالة الطباعة المباشرة على الطابعة
function printOrder() {
    ...
    printWindow.document.write(`
        ...
        <head>
            ...
            <style>
                ...
                th { background-color: #f2f2f2; text-align: right; padding: 8px; border: 1px solid #ddd; }
        ...
        <body onload="window.print(); window.close();">
            <h1>فاتورة الطلب #${<?= $order_id ?>}</h1>
            
            <div>
                <h3>معلومات العميل</h3>
                <p><span class="info-label">الاسم:</span> ${document.querySelector('.customer-info .info-item:nth-child(1)').textContent.trim()}</p>
                <p><span class="info-label">العنوان:</span> ${document.querySelector('.customer-info .info-item:nth-child(2)').textContent.trim()}</p>
                <p><span class="info-label">الهاتف:</span> ${document.querySelector('.customer-info .info-item:nth-child(3)').textContent.trim()}</p>
                <p><span class="info-label">التاريخ:</span> ${document.querySelector('.customer-info .info-item:nth-child(4)').textContent.trim()}</p>
                <p><span class="info-label">طريقة الدفع:</span> ${document.querySelector('.customer-info .info-item:nth-child(5)')?.textContent.trim() || 'غير محدد'}</p>
            </div>
            ...
            <div style="margin-top: 30px; font-weight: bold;">
                <p>المجموع الكلي: ${document.querySelector('.total-row td:last-child').textContent.trim()}</p>
            </div>
...
// دالة لإنشاء جدول المنتجات للطباعة (تبقى كما هي)
function generatePrintableTable() {
    const rows = document.querySelectorAll('.order-table tbody tr:not(.total-row)');
    let tableHtml = `
        <table>
            <thead>
                <tr>
    ...
    rows.forEach(row => {
        ...
        tableHtml += `
            <tr>
                <td class="text-center">${cells[0].textContent.trim()}</td>
                <td>${cells[2].textContent.trim()}</td>
                <td>${cells[3].textContent.trim()}</td>
                <td class="text-center">${cells[4].textContent.trim()}</td>
